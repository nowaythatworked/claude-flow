...
- If the user picks tasks directly: ensure phase is `planned` first (if currently `implementing`, run `--clear-focus` then `--set-phase planned`), then set focus via `"${CLAUDE_PLUGIN_ROOT}/scripts/session.sh" . "$CURRENT_SESSION_ID" --set-focus "<task>"` and deep-dive following the `/flow:next` deep-dive rules (pseudo-implementation, pre-plan types/schemas, trace impact)
